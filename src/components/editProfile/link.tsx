...
const Link: React.FC = () => {
  ...
  const onChangeLinkList = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    let newArr = links.introduce_link_url ? [...links.introduce_link_url] : [];
  ...
  const linkInputList = useMemo(
    () =>
      Array(linkCount - 1)
        .fill(void 0)
        .map((item, index) => {
          const value = links.introduce_link_url ? links.introduce_link_url[index + 1] : '';
          return (
            <input
              name={String(index + 1)}
              onChange={onChangeLinkList}
              value={value}
              key={index}
              className="writeLink"
            />
    ...
    <>
      ...
      <Links>
        <input
          className="writeLink"
          onChange={onChangeLinkList}
          value={links.introduce_link_url ? links.introduce_link_url[0] : ''}
          name="0"
        />
        {linkInputList}
